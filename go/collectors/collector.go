...
// CollectorResult represents the output of a single collector for streaming
...
// CollectAllStreaming collects data with callback for each collector
func CollectAllStreaming(onResult func(CollectorResult) error) *models.Report {
	report := &models.Report{
		Timestamp: time.Now(),
		Hostname:  GetHostname(),
	}
	...
	onResult(CollectorResult{Collector: "os", Timestamp: time.Now(), Data: report.OS})

	// System info (CPU, memory, load)
	report.System = CollectSystemInfo()
	onResult(CollectorResult{Collector: "system", Timestamp: time.Now(), Data: report.System})

	// Disk info
	report.Disk = CollectDiskInfo()
	onResult(CollectorResult{Collector: "disk", Timestamp: time.Now(), Data: report.Disk})

	// Network info
	report.Network = CollectNetworkInfo()
	onResult(CollectorResult{Collector: "network", Timestamp: time.Now(), Data: report.Network})

	// Packages info
	report.Packages = CollectPackageInfo()
	onResult(CollectorResult{Collector: "packages", Timestamp: time.Now(), Data: report.Packages})

	// Services info
	report.Services = CollectServiceInfo()
	onResult(CollectorResult{Collector: "services", Timestamp: time.Now(), Data: report.Services})

	// Security info
	report.Security = CollectSecurityInfo()
	onResult(CollectorResult{Collector: "security", Timestamp: time.Now(), Data: report.Security})

	// Hardware info
	report.Hardware = CollectHardwareInfo()
	onResult(CollectorResult{Collector: "hardware", Timestamp: time.Now(), Data: report.Hardware})

	// Docker info
	report.Docker = CollectDockerInfo()
	onResult(CollectorResult{Collector: "docker", Timestamp: time.Now(), Data: report.Docker})

	// Snaps info
	report.Snaps = CollectSnapInfo()
	onResult(CollectorResult{Collector: "snaps", Timestamp: time.Now(), Data: report.Snaps})

	// GPU info
	report.GPU = CollectGPUInfo()
	onResult(CollectorResult{Collector: "gpu", Timestamp: time.Now(), Data: report.GPU})

	// Logs info
	report.Logs = CollectLogInfo()
	onResult(CollectorResult{Collector: "logs", Timestamp: time.Now(), Data: report.Logs})

	// Analyze and emit issues
	report.Issues = analyzeIssues(report)
	onResult(CollectorResult{Collector: "issues", Timestamp: time.Now(), Data: report.Issues})
