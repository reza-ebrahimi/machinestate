    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="section-content">
                <table>
                    ...
                    <tbody>
                        {{range .Hardware.Temperatures}}
                        <tr>
                            <td>{{.Label}}</td>
                            <td class="{{tempStatus .Current}}">{{printf "%.1f" .Current}}°C</td>
                            <td>{{if gt .High 0.0}}{{printf "%.1f" .High}}°C{{else}}-{{end}}</td>
                            <td>{{if gt .Critical 0.0}}{{printf "%.1f" .Critical}}°C{{else}}-{{end}}</td>
                        </tr>
                        {{end}}
                    </tbody>
                </table>
        ...
        <div class="section">
            ...
            <div class="section-content">
                <div class="kv-row">
                    <span class="kv-key">Status</span>
                    <span class="kv-value {{if .Docker.DaemonRunning}}status-ok{{else}}status-error{{end}}">
                        {{if .Docker.DaemonRunning}}Running{{else}}Not Running{{end}}
                    </span>
                </div>
                {{if .Docker.DaemonRunning}}
                <div class="kv-row">
                    <span class="kv-key">Running Containers</span>
                    <span class="kv-value">{{.Docker.RunningCount}}</span>
                </div>
                <div class="kv-row">
                    <span class="kv-key">Stopped Containers</span>
                    <span class="kv-value">{{.Docker.StoppedCount}}</span>
                </div>
                <div class="kv-row">
                    <span class="kv-key">Images</span>
                    <span class="kv-value">{{.Docker.ImageCount}}</span>
                </div>
                <div class="kv-row">
                    <span class="kv-key">Total Image Size</span>
                    <span class="kv-value">{{formatBytesInt64 .Docker.TotalImageSize}}</span>
                </div>
                {{if gt .Docker.DanglingImages 0}}
                <div class="kv-row">
                    <span class="kv-key">Dangling Images</span>
                    <span class="kv-value status-warn">{{formatBytesInt64 .Docker.DanglingImages}}</span>
                </div>
                {{end}}
                {{end}}
            </div>
        ...
        <div class="section">
            ...
            <div class="section-content">
                <div class="kv-row">
                    ...
                    <span class="kv-value">{{len .Snaps.Snaps}}</span>
                </div>
                <div class="kv-row">
                    <span class="kv-key">Total Disk Usage</span>
                    <span class="kv-value">{{formatBytesInt64 .Snaps.TotalDiskUsage}}</span>
                </div>
                {{if gt .Snaps.PendingRefreshes 0}}
                <div class="kv-row">
                    <span class="kv-key">Pending Refreshes</span>
                    <span class="kv-value status-warn">{{.Snaps.PendingRefreshes}}</span>
                </div>
                {{end}}
            </div>
        </div>
            ...
            <div class="section-content">
                ...
                <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid var(--border);">
                    ...
                    <div class="kv-row">
                        ...
                        <span class="kv-value {{gpuTempStatus .Temperature}}">{{.Temperature}}°C</span>
                    </div>
                    {{end}}
                    {{if gt .Utilization 0}}
                    <div class="kv-row">
                        <span class="kv-key">Utilization</span>
                        <span class="kv-value">{{.Utilization}}%</span>
                    </div>
                    {{end}}
                    {{if gt .MemoryTotal 0}}
                    <div class="kv-row">
                        <span class="kv-key">Memory</span>
                        <span class="kv-value">{{formatBytes .MemoryUsed}} / {{formatBytes .MemoryTotal}}</span>
                    </div>
                    {{end}}
                    {{if gt .PowerDraw 0.0}}
                    <div class="kv-row">
                        <span class="kv-key">Power</span>
                        <span class="kv-value">{{printf "%.1f" .PowerDraw}}W</span>
                    </div>
                    {{end}}
                </div>
                {{end}}
        ...
        {{if .Logs.Available}}
        <div class="section">
            <div class="section-title">Logs (24h)</div>
            <div class="section-content">
                <div class="kv-row">
                    <span class="kv-key">Errors</span>
                    <span class="kv-value {{if gt .Logs.Stats.ErrorCount 0}}status-warn{{else}}status-ok{{end}}">{{.Logs.Stats.ErrorCount}}</span>
                </div>
                <div class="kv-row">
                    <span class="kv-key">Warnings</span>
                    <span class="kv-value">{{.Logs.Stats.WarningCount}}</span>
                </div>
                {{if gt .Logs.Stats.CriticalCount 0}}
                <div class="kv-row">
                    <span class="kv-key">Critical</span>
                    <span class="kv-value status-error">{{.Logs.Stats.CriticalCount}}</span>
                </div>
                {{end}}
                {{if gt .Logs.Stats.OOMEvents 0}}
                <div class="kv-row">
                    <span class="kv-key">OOM Events</span>
                    <span class="kv-value status-error">{{.Logs.Stats.OOMEvents}}</span>
                </div>
                {{end}}
                {{if gt .Logs.Stats.KernelPanics 0}}
                <div class="kv-row">
                    <span class="kv-key">Kernel Panics</span>
                    <span class="kv-value status-error">{{.Logs.Stats.KernelPanics}}</span>
                </div>
                {{end}}
                {{if gt .Logs.Stats.Segfaults 0}}
                <div class="kv-row">
                    <span class="kv-key">Segfaults</span>
                    <span class="kv-value status-error">{{.Logs.Stats.Segfaults}}</span>
                </div>
                {{end}}
            </div>
        </div>
...
func RenderHTML(report *models.Report) (string, error) {
	funcMap := template.FuncMap{
		...
		"formatBytesInt64": func(b int64) string {
			if b < 0 {
				return "0 B"
			}
		...
		"tempStatus": func(t float64) string {
			if t >= 85 {
				return "status-error"
			} else if t >= 70 {
				return "status-warn"
		...
		"gpuTempStatus": func(t int) string {
			if t >= 90 {
				return "status-error"
			} else if t >= 80 {
				return "status-warn"
